...
(SELECT COUNT(*) FROM cvc_order_info io WHERE io.batch_no= cgos.unified_batch_no AND io.exception_status > 0 AND io.is_show = 1) as exception_count,
(SELECT COUNT(*) FROM cvc_order_info io WHERE io.batch_no= cgos.unified_batch_no AND io.exception_status > 0 AND io.is_show = 2) as exception_do_count,
