 ...
 */
@Controller
@RequestMapping("/hotel/cvcShippingBatch")
public class CvcShippingBatchController extends BaseController{
  ...
  private CvcShippingBatchService cvcShippingBatchService;
	...
	public void list(@ModelAttribute CvcShippingBatchEntity query,HttpServletRequest request,HttpServletResponse response,
			@RequestParam(required = false, value = "pageNo", defaultValue = "1") int pageNo,
			@RequestParam(required = false, value = "pageSize", defaultValue = "10") int pageSize) throws Exception{
			try {
			 	LOG.info(request, " back list");
			 	//分页数据
				MiniDaoPage<CvcShippingBatchEntity> list =  cvcShippingBatchService.getAll(query,pageNo,pageSize);
				VelocityContext velocityContext = new VelocityContext();
				velocityContext.put("cvcShippingBatch",query);
				velocityContext.put("pageInfos",SystemTools.convertPaginatedList(list));
				String viewName = "hotel/shippingbatch/cvcShippingBatch-list.vm";
	...
	public AjaxJson doDelete(@RequestParam(required = true, value = "id" ) String id){
			AjaxJson j = new AjaxJson();
			try {
				cvcShippingBatchService.delete(id);
				j.setMsg("删除成功");
			} catch (Exception e) {
			    log.info(e.getMessage());
				j.setSuccess(false);
